...
export default function FUITestimonialWithGrid() {
  return (
    <section
      ...
      className=" relative bg-transpaarent py-20 sm:py-10 max-w-screen z-1 bg-page-gradient"
    >
      ...
      <Container>
        <div className="mx-auto max-w-2xl md:text-center">
          <h2 className="text-3xl  font-bold mb-4  tracking-wide  tex-center  bg-clip-text bg-[linear-gradient(180deg,_#FFF_0%,_rgba(255,_255,_255,_0.00)_202.08%)] text-transparent  mx-auto md:text-5xl">
            <span className="bg-gradient-to-br from-indigo-400 via-indigo-300 to-indigo-700 bg-clip-text text-transparent">
              Loved
            </span>{" "}
            by businesses worldwide.
          </h2>
          <p className="mt-4 text-gradient text-xl">
        ...
        >
          {testimonials.map((column, columnIndex) => (
            <li key={columnIndex} className="transition-all">
              <ul role="list" className="flex flex-col gap-y-6 sm:gap-y-8">
                {column.map((testimonial, testimonialIndex) => (
                  <li key={testimonialIndex}>
                    <figure className="relative rounded-2xl bg-transparent transform-gpu [border:1px_solid_rgba(255,255,255,.1)] [box-shadow:0_-20px_80px_-20px_#8686f01f_inset] p-6 shadow-xl transition-transform duration-300 ease-out hover:-translate-y-3 hover:shadow-2xl">
